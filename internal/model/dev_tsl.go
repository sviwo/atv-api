...
type TSLEvent struct {
	Key      string           `json:"identifier" dc:"事件标识" v:"required|regex:^[A-Za-z_]+[\\w]*$#请输入事件标识|标识由字母、数字和下划线组成,且不能以数字开头"`
	Name     string           `json:"name" dc:"事件名称" v:"required#请输入事件名称"`
	Type     string           `json:"type" dc:"事件级别:info=普通,alert=警告,exigency=紧急" v:"required#请选择事件级别"`
	Required bool             `json:"required" dc:"是否必填"`
	Outputs  []TSLEventOutput `json:"outputData" dc:"输出参数"`
	Method   string           `json:"method" dc:"方式"`
...
type TSLEventOutput struct {
	...
	ValueType TSLValueType `json:"dataType" dc:"参数值"`
}
