...
type AppMediaTreeReq struct {
	...
	PageType int `json:"pageType"    v:"required"        dc:"页面类型：0=帮助页，1=注册用户页，2=服务页"`
}

type AppMediaTreeRes struct {
	Id          string             `json:"id"          dc:""`
	ParentId    string             `json:"parentId"    dc:""`
	DisplayType int                `json:"displayType" dc:"显示类型：0=直接显示，1=跳转外链"`
	Title       string             `json:"title"       dc:"标题"`
	MediaDesc   string             `json:"mediaDesc"   dc:"简介"`
	SmallImg    string             `json:"smallImg"    dc:"缩略图"`
	Icon        string             `json:"icon"        dc:"icon"`
	Content     string             `json:"content"     dc:"内容（当显示类型为0的时候content为空，为1的时content为链接地址）"`
