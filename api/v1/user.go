...
type RegisterReq struct {
	...
	EmailVftCode    string `json:"emailVftCode"    dc:"邮箱验证码（纯数字，长度必须6位）"              v:"required|integer|size:6"`
}
...
type UpdatePasswordReq struct {
	...
	Username        string `json:"username"          dc:"用户名（标准邮箱格式）"                         v:"required|email"`
	NewPassword     string `json:"newPassword"       dc:"新密码（长度在6~18之间，必须包含大小写字母和数字）" v:"required|password2"`
	ConfirmPassword string `json:"confirmPassword"   dc:"确认密码（必须与新密码一致）"                    v:"required|eq:NewPassword"`
	EmailVftCode    string `json:"emailVftCode"      dc:"邮箱验证码（纯数字，长度必须6位）"                v:"required|integer|size:6"`
}
